...
#### Generalizing a bit...
Our bartender could ask a different question: "Do you not want a beer"? To be
really general, I'll allow two more questions: 1) "Is 1 equal to 1?"
...
levelplot(
  m,
  col.regions=colorRampPalette(colors=c("red", "yellow", "green")),
  main = list('Generalized Question', side=1, line=0.5),
  xlab="respondent",
  ylab="Question/A and B's Desires",
  colorkey=list(at=c(-1, -.33, .33, 1), labels=list(at=c(-0.67, 0, 0.67), labels=c("No", "I don't know", "Yes!"))),
  scales=list(
    alternating=1,
    x=list(at=1:1, labels=c("A")),
...
#### Generalizing a bit...
If we add one more logician, a new outcome appears: uncertainty. The basic "do you want a beer?"
becomes "do you both want a beer?", with this outcome:
...
In this case A and B both want a beer: this is their desire, but doesn't necessarily
indicate how they answer the question. Instead, the answers are in the grid, left to right: for the
top row, the answers are "A: I don't know; B: Yes!"). With two logicians, I think
...
can find the answer online, spoiler alert, it's $2^{2^{2}} = 16$. One way to think about it: imagine a template for a
